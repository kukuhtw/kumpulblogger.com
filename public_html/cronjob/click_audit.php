...
echo "<br>isUsingProxyOrVpnHeaders: " . (isset($_SERVER['HTTP_FORWARDED']) ? $_SERVER['HTTP_FORWARDED'] : 'Tidak ada header');
    ...
    if (countClicks($click['ip_address'], $click['user_cookies'], $click['browser_agent'], 20, $mysqli) > $aj) {
        $reason = 'Threshold AJ exceeded '.$aj.' ( rentang Jarak 20 detik terakhir dihitung dari `time_epoch_click`  antara klik terekam ada IP address dan browser yang sama) ';
        auditClick($click['id'], $reason, 1, $mysqli);
        banIpForVelocity($click['ip_address'], $reason, $mysqli);
        return;
    }


    if (countClicks($click['ip_address'], null, $click['browser_agent'], 60, $mysqli) > $aa) {
        $reason = 'Threshold AA exceeded '.$aa.' ( rentang Jarak 1 menit terakhir dihitung dari `time_epoch_click`  antara klik terekam ada IP address dan browser yang sama) ';
        auditClick($click['id'], $reason, 1, $mysqli);
        banIpForVelocity($click['ip_address'], $reason, $mysqli);
        return;
    }
    ...
    if (countClicks($click['ip_address'], null, $click['browser_agent'], 120, $mysqli) > $ab) {
        $reason = 'Threshold AB exceeded '.$ab.' (rentang Jarak 2 menit terakhir dihtung dari `time_epoch_click `antara klik terekam ada  IP address dan browser yang sama)';
        auditClick($click['id'], $reason, 1, $mysqli);
        banIpForVelocity($click['ip_address'], $reason, $mysqli);
        return;
    }
    ...
    if (countClicks($click['ip_address'], null, $click['browser_agent'], 300, $mysqli) > $ac) {
        $reason = 'Threshold AC exceeded '.$ac.' (rentang Jarak 5 menit terakhir dihtung dari `time_epoch_click `antara klik terekam ada  IP address dan browser yang sama)';
        auditClick($click['id'], $reason, 1, $mysqli);
        banIpForVelocity($click['ip_address'], $reason, $mysqli);
        return;
    }
    ...
    if (countClicks($click['ip_address'], $click['user_cookies'], $click['browser_agent'], 600, $mysqli) > $ad) {
        $reason = 'Threshold AD exceeded '.$ad.' (rentang Jarak 10 menit terakhir dihtung dari `time_epoch_click `antara klik terekam ada  IP address dan browser yang sama)';
        auditClick($click['id'], $reason, 1, $mysqli);
        banIpForVelocity($click['ip_address'], $reason, $mysqli);
        return;
    }
    ...
    if (countClicks($click['ip_address'], null, $click['browser_agent'], 900, $mysqli) > $ae) {
        $reason = 'Threshold AE exceeded '.$ae.' (rentang Jarak 15 menit terakhir dihtung dari `time_epoch_click `antara klik terekam ada  IP address dan browser yang sama)';
        auditClick($click['id'], $reason, 1, $mysqli);
        banIpForVelocity($click['ip_address'], $reason, $mysqli);
        return;
    }
    ...
    if (countClicks($click['ip_address'], null, $click['browser_agent'], 1200, $mysqli) > $af) {
        $reason = 'Threshold AF exceeded '.$af.' (rentang Jarak 20 menit terakhir dihtung dari `time_epoch_click `antara klik terekam ada  IP address dan browser yang sama)';
        auditClick($click['id'], $reason, 1, $mysqli);
        banIpForVelocity($click['ip_address'], $reason, $mysqli);
        return;
    }
    ...
    if (countClicks($click['ip_address'], $click['user_cookies'], $click['browser_agent'], 1500, $mysqli) > $ag) {
        $reason = 'Threshold AG exceeded '.$ag.' (rentang Jarak 25 menit terakhir dihtung dari `time_epoch_click `antara klik terekam ada  IP address dan browser yang sama)';
        auditClick($click['id'], $reason, 1, $mysqli);
        banIpForVelocity($click['ip_address'], $reason, $mysqli);
        return;
    }
    ...
    if (countClicks($click['ip_address'], null, $click['browser_agent'], 1800, $mysqli) > $ah) {
        $reason = 'Threshold AH exceeded '.$ah.'  (rentang Jarak 30  menit terakhir dihtung dari `time_epoch_click `antara klik terekam ada  IP address dan browser yang sama)';
        auditClick($click['id'], $reason, 1, $mysqli);
        banIpForVelocity($click['ip_address'], $reason, $mysqli);
        return;
    }
    ...
    if (countClicks($click['ip_address'], null, $click['browser_agent'], 2100, $mysqli) > $ai) {
        $reason = 'Threshold AI exceeded '.$ai.' (rentang Jarak 35 menit terakhir dihtung dari `time_epoch_click `antara klik terekam ada  IP address dan browser yang sama)';
        auditClick($click['id'], $reason, 1, $mysqli);
        banIpForVelocity($click['ip_address'], $reason, $mysqli);
        return;
    }
     ...
     if (countClicks($click['ip_address'], null, $click['browser_agent'], 3600 , $mysqli) > $ak) {
        $reason = 'Threshold AK exceeded '.$ai.' (rentang Jarak 1 jam terakhir dihtung dari `time_epoch_click `antara klik terekam ada  IP address dan browser yang sama)';
        auditClick($click['id'], $reason, 1, $mysqli);
        banIpForVelocity($click['ip_address'], $reason, $mysqli);
        return;
    }
    ...
    if (countClicks($click['ip_address'], null, $click['browser_agent'], 7200, $mysqli) > $al) {
        $reason = 'Threshold 2 jam exceeded';
        auditClick($click['id'], $reason, 1, $mysqli);
        banIpForVelocity($click['ip_address'], $reason, $mysqli);
        return;
    }
    ...
    if (countClicks($click['ip_address'], null, $click['browser_agent'], 14400, $mysqli) > $am) {
        $reason = 'Threshold 4 jam exceeded';
        auditClick($click['id'], $reason, 1, $mysqli);
        banIpForVelocity($click['ip_address'], $reason, $mysqli);
        return;
    }
    ...
    if (countClicks($click['ip_address'], null, $click['browser_agent'], 21600, $mysqli) > $an) {
        $reason = 'Threshold 6 jam exceeded';
        auditClick($click['id'], $reason, 1, $mysqli);
        banIpForVelocity($click['ip_address'], $reason, $mysqli);
        return;
    }
    ...
    if (countClicks($click['ip_address'], null, $click['browser_agent'], 43200, $mysqli) > $ao) {
        $reason = 'Threshold 12 jam exceeded';
        auditClick($click['id'], $reason, 1, $mysqli);
        banIpForVelocity($click['ip_address'], $reason, $mysqli);
        return;
    }
    ...
    if (countClicks($click['ip_address'], null, $click['browser_agent'], 86400, $mysqli) > $ap) {
        $reason = 'Threshold 24 jam exceeded';
        auditClick($click['id'], $reason, 1, $mysqli);
        banIpForVelocity($click['ip_address'], $reason, $mysqli);
        return;
    }
...
function isBrowserAgentBot($browser_agent) {
    // Delegates to the shared, more thorough check in function.php (covers
    // common HTTP-library/automation-tool signatures and missing/empty UA,
    // not just a handful of crawler keywords).
    return is_probable_bot_user_agent($browser_agent);
}
...
// Auto-ban an IP the first time it trips a click-velocity threshold, so a
// repeat offender's *next* burst gets blocked from its very first click
// (via the list_ip_banned check in show_ads_native*.js.php and isIpBanned()
// above) instead of only being caught once the pattern repeats again from
// scratch. Only called for velocity-threshold rejections — not for reasons
// like "Ad is expired" or proxy/VPN heuristics, which aren't evidence the
// IP itself is a repeat click-fraud offender.
function banIpForVelocity($ip_address, $reason, $mysqli) {
    $check_stmt = $mysqli->prepare("SELECT COUNT(*) FROM list_ip_banned WHERE ip_address = ?");
    $check_stmt->bind_param("s", $ip_address);
    $check_stmt->execute();
    $check_stmt->bind_result($already_banned);
    $check_stmt->fetch();
    $check_stmt->close();
    ...
    $ban_reason = 'Auto-ban (velocity threshold): ' . $reason;
    $ban_stmt = $mysqli->prepare("INSERT INTO list_ip_banned (ip_address, reason, date_banned) VALUES (?, ?, ?)");
    $ban_stmt->bind_param("sss", $ip_address, $ban_reason, $date_banned);
    $ban_stmt->execute();
    $ban_stmt->close();

    echo "<br><strong style='color:red;'>IP $ip_address otomatis di-ban (velocity threshold terlampaui).</strong>";
}
