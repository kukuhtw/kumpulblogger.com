...
$page = max(1, isset($_GET['page']) && is_numeric($_GET['page']) ? intval($_GET['page']) : 1);
...
<head>
    ...
    <style>
        body { margin: 0; background: #f5f7fa; color: #26313c; }
        ...
        .site-card { overflow: hidden; border: 0; border-radius: .9rem; box-shadow: 0 3px 14px rgba(31,41,55,.08); transition: transform .2s ease, box-shadow .2s ease; }
        .site-card:hover { transform: translateY(-3px); box-shadow: 0 7px 22px rgba(31,41,55,.12); }
        body.modal-open .site-card, body.modal-open .site-card:hover { transform: none; transition: none; }
        .site-card .card-body { display: flex; flex-direction: column; padding: 1.2rem; }
        .site-card-title { margin-bottom: .2rem; font-size: 1.15rem; font-weight: 750; overflow-wrap: anywhere; }
        .site-domain { display: block; margin-bottom: 1rem; color: #0d6efd; font-size: .82rem; overflow-wrap: anywhere; }
        .site-description { min-height: 2.8rem; color: #64707c; font-size: .88rem; }
        .site-info { display: grid; gap: .45rem; margin: .5rem 0 1rem; }
        .site-info-row { display: flex; justify-content: space-between; gap: 1rem; padding-bottom: .4rem; border-bottom: 1px dashed #e1e6eb; font-size: .82rem; }
        .site-info-row span:last-child, .site-info-row strong { max-width: 65%; text-align: right; overflow-wrap: anywhere; }
        .revenue-box { margin-bottom: 1rem; padding: .8rem; border-radius: .65rem; background: #eaf8f0; color: #12683a; }
        .revenue-box small { display: block; font-weight: 700; text-transform: uppercase; }
        .revenue-box a { color: inherit; font-size: 1.2rem; font-weight: 800; text-decoration: none; }
        .site-card > .card-body > .btn { width: 100%; margin-bottom: .5rem !important; }
        .site-pagination { flex-wrap: wrap; justify-content: center; gap: .25rem; margin: 1.5rem 0 0; }
        .site-pagination .page-link { min-width: 38px; border-radius: .45rem !important; text-align: center; }
...
<div class="container page-shell py-3 py-md-4">
    ...
    <?php echo $flash_message_html; ?>

    <div class="page-heading d-flex flex-column flex-sm-row align-items-sm-center justify-content-between gap-3 mb-4">
        <div><h1>Site Publisher Saya</h1><p>Kelola konfigurasi, script iklan, pendapatan, dan iklan pada setiap site.</p></div>
        <a href="add_site.php" class="btn btn-primary"><i class="fas fa-plus-circle me-1"></i> Tambah Site</a>
    </div>
                    ...
                    <div class="card site-card h-100">
                        <div class="card-body">
                            <div class="d-flex justify-content-between align-items-start gap-2">
                                <h2 class="site-card-title"><?php echo htmlspecialchars($publisher['site_name']); ?></h2>
                                <span class="badge <?php echo $publisher['isbanned'] ? 'text-bg-danger' : 'text-bg-success'; ?>"><?php echo $publisher['isbanned'] ? 'Banned' : 'Aktif'; ?></span>
                            </div>
                            <a class="site-domain" href="<?php echo htmlspecialchars($publisher['site_domain'], ENT_QUOTES); ?>" target="_blank" rel="noopener noreferrer"><?php echo htmlspecialchars($publisher['site_domain']); ?></a>
                            <p class="site-description"><?php echo htmlspecialchars($publisher['site_desc']); ?></p>
                            <div class="site-info">
                                <div class="site-info-row"><span>Publisher ID</span><strong><?php echo htmlspecialchars($publisher['id']); ?></strong></div>
                                <div class="site-info-row"><span>Provider</span><span><?php echo htmlspecialchars($publisher['providers_name']); ?></span></div>
                                <div class="site-info-row"><span>Rate Publisher</span><strong>Rp <?php echo number_format((float) $publisher['rate_text_ads'], 0, ',', '.'); ?></strong></div>
                                <div class="site-info-row"><span>Harga Lokal</span><strong>Rp <?php echo number_format($rate_text_ads_margin_local, 0, ',', '.'); ?></strong></div>
                                <div class="site-info-row"><span>Harga Partner</span><strong>Rp <?php echo number_format($rate_text_ads_margin_partner, 0, ',', '.'); ?></strong></div>
                                <div class="site-info-row"><span>Advertiser Diizinkan</span><span><?php echo htmlspecialchars($publisher['advertiser_allowed'] ?: '-'); ?></span></div>
                                <div class="site-info-row"><span>Advertiser Ditolak</span><span><?php echo htmlspecialchars($publisher['advertiser_rejected'] ?: '-'); ?></span></div>
                            </div>
                            <div class="revenue-box"><small>Pendapatan Publisher</small><a href="<?php echo htmlspecialchars($link_report, ENT_QUOTES); ?>">Rp <?php echo number_format($revenuePublishers, 0, ',', '.'); ?></a></div>
                            <div class="d-none">
    ...
    <i class="fas fa-edit me-1"></i> Edit Site
               ...
               <button class="btn btn-outline-dark mb-2" data-bs-toggle="modal" data-bs-target="#takeScriptModal-<?php echo $publisher['id']; ?>"
    data-id="<?php echo $publisher['id']; ?>">
    <i class="fas fa-code me-1"></i> Ambil Script
</button>
      ...
      <div class="modal-footer">
        ...
        <button type="button" class="btn btn-primary" onclick="copyToClipboard('script2-<?php echo $publisher['id']; ?>')">Copy Script</button>
                            ...
                            <a href="mysite_ads.php?publisher_site_local_id=<?php echo $publisher['id']; ?>" class="btn btn-info mb-2">
                                <i class="fas fa-ad me-1"></i> Lihat Iklan Site
                            </a>
...
?>
<button class="btn btn-outline-danger" type="button" disabled aria-disabled="true" title="Fitur hapus site dinonaktifkan">
    <i class="fas fa-trash-alt me-1"></i> Hapus Site (Dinonaktifkan)
</button>
<?php
...
?>
    ...
    <nav aria-label="Navigasi daftar site">
      <ul class="pagination site-pagination">
        <li class="page-item <?php echo $page <= 1 ? 'disabled' : ''; ?>"><a class="page-link" href="?page=<?php echo max(1, $page - 1); ?>">&laquo;</a></li>
        ...
        <?php endif; ?>
        <li class="page-item <?php echo $page >= $total_pages ? 'disabled' : ''; ?>"><a class="page-link" href="?page=<?php echo min($total_pages, $page + 1); ?>">&raquo;</a></li>
      </ul>
    </nav>
</div>
...
<!-- Bootstrap JS -->
<script>
// Modal yang berada di dalam card ber-transform dapat mengalami flicker.
// Pindahkan ke body agar position: fixed dan backdrop Bootstrap stabil.
document.querySelectorAll('.site-card .modal').forEach(function (modal) {
