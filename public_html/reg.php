...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($email_count > 0) {
        ...
        if ($stmt->execute()) {
            $send_email = 1;
            echo "<div class='alert alert-success'>Registrasi berhasil! Password Anda: " . htmlspecialchars($passwords) . ".
                <form action='login.php' method='POST' class='d-inline'>
                    <input type='hidden' name='prefill' value='1'>
                    <input type='hidden' name='email' value='" . htmlspecialchars($loginemail) . "'>
                    <input type='hidden' name='password' value='" . htmlspecialchars($passwords) . "'>
                    <button type='submit' class='btn btn-link p-0 align-baseline'>Lanjut ke login page</button>
                </form>
            </div>";
        } else {
