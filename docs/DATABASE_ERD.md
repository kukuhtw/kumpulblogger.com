# Dokumentasi ERD Database — `kumpulbl_kbc`

> Sumber kebenaran struktur: `sql/kumpulbl_kbc_hanya_structure.sql` (dump phpMyAdmin, 30 tabel).
...
- Kolom lengkap tiap tabel (constraint `NOT NULL`, default value, tipe presisi) ada di `sql/kumpulbl_kbc_hanya_structure.sql` — dokumen ini hanya menampilkan kolom yang relevan untuk memahami relasi.
