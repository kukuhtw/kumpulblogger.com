...
> Navigasi: [Runbook operasional](../OPERATIONS_RUNBOOK.md) · [Gambaran bisnis](./01-gambaran-umum-bisnis.md) · [Publisher](./03-alur-publisher.md) · [Advertiser](./04-alur-advertiser.md)

## 1. User terdaftar (Publisher / Advertiser) — tabel `msusers`

Aplikasi ini **tidak memisahkan akun Publisher dan Advertiser**. Semua pengguna publik yang mendaftar lewat `public_html/reg.php` masuk ke satu tabel `msusers` (`sql/myadnetwork_db_hanya_structure.sql:451-478`). Setelah login, dashboard menu (`public_html/main_menu.php:22-23`) menampilkan dua tombol menu terpisah — **Advertiser** dan **Publisher** — yang keduanya bisa diakses oleh akun yang sama. Peran ditentukan murni oleh *tindakan* yang dilakukan akun tersebut:
...
Superuser platform, terpisah total dari `msusers` (skema di `sql/myadnetwork_db_hanya_structure.sql:432-443`), login lewat `public_html/admin/login.php` dengan session key berbeda (`$_SESSION['loggedin']`, `$_SESSION['loginemail_admin']` — dipakai konsisten di semua file `public_html/admin/*.php`). Admin **tidak** menggunakan `msusers`, sehingga tidak bisa sekaligus jadi publisher/advertiser biasa lewat akun admin yang sama.
