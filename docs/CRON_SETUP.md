...
> Terkait: [CRONJOB_JOBS.md](./CRONJOB_JOBS.md) (penjelasan detail per file), [API_ENDPOINTS.md](./API_ENDPOINTS.md) (federasi provider/partner).
>
> Dokumen ini melengkapi bagian "Perlu konfirmasi" di `CRONJOB_JOBS.md`/`documentation/11-cronjob-dan-otomatisasi.md` yang sebelumnya menyebut "tidak ada file crontab di repo, jadwal perlu konfirmasi ke tim operasional" — jadwal di bawah ini adalah contoh nyata yang sudah dipakai di produksi.
...
Jadwal di atas hanya mencakup jalur **lokal** (iklan & publisher milik provider sendiri). Kalau instance Anda sudah/akan terhubung dengan jaringan partner lain (lihat alur federasi di [API_ENDPOINTS.md](./API_ENDPOINTS.md) §5), job-job berikut **juga perlu dijadwalkan** — tidak ada di contoh di atas kemungkinan karena instance sumber jadwal ini belum (atau belum lagi) punya partner aktif:
