...
- **Kunci bisnis** (dipakai untuk cek "sudah ada/belum"): `local_ads_id` + `publishers_site_local_id` + `ads_providers_domain_url`. ✅ Sekarang dilindungi `UNIQUE KEY uniq_ads_site_domain` di skema (sebelumnya hanya `PRIMARY KEY (id)` dan beberapa `KEY` non-unique untuk `local_ads_id`/`publishers_site_local_id`/`ads_providers_domain_url`/`pubs_providers_domain_url` secara terpisah — lihat `sql/kumpulbl_kbc_hanya_structure.sql:1031-1041`). Untuk instalasi yang sudah berjalan, terapkan lewat migrasi `sql/2026_07_27_dedupe_and_unique_mapping_ads_publisher_site.sql` (membersihkan duplikat lama dulu sebelum menambahkan constraint). Detail & riwayat perbaikan di §6 #4.
...
**Tahap 2 — dari sisi iklan advertiser.** Untuk tiap `advertisers_ads`, ambil mapping-nya **`WHERE local_ads_id = ? AND ads_providers_domain_url = ?`** (✅ filter domain ditambahkan — sebelumnya hanya `local_ads_id` tanpa domain, berisiko tabrakan dengan mapping milik iklan partner yang kebetulan punya `local_ads_id` sama; lihat riwayat perbaikan di §6 #3). Sebelum cek harga, kalau status master iklan tidak aktif (`is_paid=0` atau `ispublished=0` atau `is_expired=1` atau `is_paused=1`), **sinkronkan langsung** `is_published`/`is_expired`/`expired_date`/`is_paused`/`paused_date` di baris mapping mengikuti status master (supaya skrip ad-serving publik tidak menunggu `push_sync_ads_expired.php`). Baru kemudian cek harga:
...
- ✅ **Diperbaiki** — Tahap 2 sebelumnya mengambil mapping lewat `WHERE local_ads_id = ?` tanpa filter domain (sama seperti versi lokal). Sekarang ditambahkan `AND ads_providers_domain_url = ?` (nilai dari `advertisers_ads_partners.providers_domain_url`, yaitu domain server asal partner tempat iklan itu dibuat) — lihat riwayat perbaikan di §6 #3.
...
| Lookup mapping by iklan di Tahap 2 check-rate | ✅ **Diperbaiki** — dulu `WHERE local_ads_id = ?` tanpa domain; sekarang `AND ads_providers_domain_url = ?` ditambahkan | ✅ **Diperbaiki** — perbaikan yang sama diterapkan |
| Locking antar-run cron (mesin pencocokan awal) | ✅ **Diperbaiki** — `GET_LOCK('mapping_ads_publisher_local', 0)` di awal skrip | ✅ **Diperbaiki** — `GET_LOCK('mapping_ads_publisher_partner', 0)` di awal skrip |
...
flowchart TD
    subgraph LOKAL[Sisi Lokal]
        A0["✅ GET_LOCK('mapping_ads_publisher_local', 0)\ngagal ambil lock -> stop (cegah run tumpang tindih)"] --> A1[advertisers_ads\nispublished=1 AND is_expired=0]
        A1 --> A2{Untuk setiap\npublishers_site}
    ...
    subgraph LOKALCHECK[Re-validator Lokal - berkala]
        ...
        B4["Tahap 2: per advertisers_ads\nbudget vs rate_text_ads mapping\n✅ lookup local_ads_id + ads_providers_domain_url"] -->|harga jelek| B5[is_approved_by_publisher=0]
    ...
    subgraph PARTNER[Sisi Partner]
        C0["✅ GET_LOCK('mapping_ads_publisher_partner', 0)\ngagal ambil lock -> stop (cegah run tumpang tindih)"] --> C1[advertisers_ads_partners\nispublished=1 AND is_expired=0]
        C1 --> C2{Untuk setiap\npublishers_site}
    ...
    subgraph PARTNERCHECK[Re-validator Partner - berkala]
        ...
        D4["Tahap 2: per advertisers_ads_partners\n✅ lookup local_ads_id + ads_providers_domain_url"] -->|harga jelek| D5[is_approved_by_publisher=0]
...
```mermaid
flowchart TD
    START([Iklan aktif + situs baru\nharga cocok - gerbang markup]) --> INS[INSERT pertama kali\nauto-approve dua arah\nTANPA tinjauan manual]
    INS --> APPR((Approved))

    APPR -->|check-rate: harga jadi\ntidak cocok| REJH((Rejected - harga))
    APPR -->|Publisher/Advertiser\nreject manual via dashboard| REJM((Rejected - manual/konten))

    REJH -->|LOKAL: check-rate re-approve\notomatis begitu harga cocok lagi| APPR
    REJH -->|PARTNER: check-rate_partner sekarang\njuga re-approve otomatis - diperbaiki| APPR

    REJM -->|LOKAL: mesin pencocokan awal TIDAK\nmenyentuh flag approval - tetap sticky| REJM
    REJM -->|PARTNER: mesin pencocokan partner sekarang\nTIDAK lagi reset paksa - diperbaiki| REJM

    APPR --> SERVE[Ditampilkan ke show_ads_native*.js.php\ndi luar cakupan dokumen ini]
```
...
3. ✅ **Diperbaiki — potensi tabrakan ID lintas-jaringan di lookup Tahap 2 kedua file check-rate.** `mapping_ads_publisher_check_rate.php` (Tahap 2) dan `mapping_ads_publisher_check_rate_partner.php` (Tahap 2) sebelumnya sama-sama mengambil mapping lewat `WHERE local_ads_id = ?` **tanpa** menyertakan `ads_providers_domain_url` dalam filter — padahal `local_ads_id` hanyalah PK auto-increment di server asalnya masing-masing (`advertisers_ads.id`/`advertisers_ads_partners.local_ads_id` yang disalin dari `id` server partner, lihat `add_advertisement.php` dan skema `advertisers_ads_partners` — keduanya mulai dari 1 dan bertambah independen). Karena tabel mapping menyimpan baris untuk **kedua** sumber (lokal dan partner) sekaligus dan `local_ads_id` kecil sangat mungkin bertabrakan antar-jaringan, query di kedua file ini berisiko memproses baris mapping milik jaringan yang salah. **Sudah diperbaiki**: kedua query Tahap 2 sekarang menyertakan `AND ads_providers_domain_url = ?` (nilai diambil dari `advertisers_ads.providers_domain_url` untuk versi lokal, `advertisers_ads_partners.providers_domain_url` untuk versi partner — kolom ini sudah ada di skema, sebelumnya hanya belum ikut di-fetch/di-filter), sehingga masing-masing skrip hanya menyentuh baris mapping milik jaringan yang benar-benar sedang divalidasinya.
4. ✅ **Diperbaiki — tidak ada UNIQUE constraint di level database untuk kunci bisnis mapping** (`local_ads_id` + `publishers_site_local_id` + `ads_providers_domain_url`) — sebelumnya hanya index biasa (lihat §2), sehingga pola *check-then-insert* di `mapping_ads_publisher.php`/`_partner.php` murni bergantung pada logika aplikasi untuk mencegah duplikat, dan dua eksekusi yang tumpang tindih (mis. cron sebelumnya belum selesai saat yang berikutnya mulai) berisiko membuat baris mapping duplikat untuk kombinasi iklan×situs yang sama. **Sudah diperbaiki dengan dua lapis**: (a) di level kode, kedua skrip mesin pencocokan awal sekarang mengambil MySQL named lock (`GET_LOCK('mapping_ads_publisher_local'/'mapping_ads_publisher_partner', 0)`) di awal eksekusi — kalau instance lain masih memegang lock yang sama, skrip langsung berhenti tanpa melakukan apa pun, alih-alih ikut jalan dan berpotensi balapan; (b) di level skema, `sql/kumpulbl_kbc_hanya_structure.sql` sekarang mendefinisikan `UNIQUE KEY uniq_ads_site_domain (local_ads_id, publishers_site_local_id, ads_providers_domain_url)` untuk instalasi baru. Untuk instalasi yang **sudah berjalan**, migrasi terpisah `sql/2026_07_27_dedupe_and_unique_mapping_ads_publisher_site.sql` disediakan untuk membersihkan baris duplikat yang mungkin sudah terlanjur ada lalu menambahkan UNIQUE KEY yang sama — migrasi ini **perlu dijalankan manual** terhadap database produksi (tidak ada migration runner di codebase ini, dan sesi ini tidak punya akses ke database live manapun untuk menjalankannya).
