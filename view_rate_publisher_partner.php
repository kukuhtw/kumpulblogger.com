...
$sql = "SELECT * 
        FROM `publishers_site_partners`
        WHERE `providers_domain_url` != ?
        AND (`site_name` LIKE ? OR `site_domain` LIKE ?)
        ORDER BY `rate_text_ads` $sort_order
        LIMIT ? OFFSET ?";
...
// Bind parameters: 2 search fields and pagination limits
$stmt->bind_param('sssii', $this_providers_domain_url, $search_param, $search_param, $limit, $offset);
...
$count_sql = "SELECT COUNT(`site_name`) AS total 
              FROM `publishers_site_partners`
              WHERE `providers_domain_url` != ? 
              AND (`site_name` LIKE ? OR `site_domain` LIKE ?)";
              
$count_stmt = $conn->prepare($count_sql);
$count_stmt->bind_param('sss', $this_providers_domain_url, $search_param, $search_param);
    ...
    <div class="container mt-5">
        ...
        <form class="mb-4" method="GET" action="view_rate_publisher.php">
            <div class="input-group">
                <input type="text" name="search" class="form-control" placeholder="Search by site name or domain..." value="<?php echo htmlspecialchars($search); ?>">
                ...
                <?php if ($result->num_rows > 0): ?>
                    ...
                    ?></td>
                         ...
                         <td><?php 
                            ...
                            echo "Rp ".number_format($rate_text_ads_margin_partner,1); 
