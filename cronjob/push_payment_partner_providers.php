<?php
// cronjob/push_payment_partner_providers.php
...
try {
    ...
    foreach ($providers as $provider) {
        // Construct the API URL by appending "/getinfoPaymentProviderPartner/index.php" to the api_endpoint
        $api_url = $provider['api_endpoint'] . "/getinfoPaymentProviderPartner/index.php";
        ...
        // Prepare the SQL query to select data from payment_partner_pubs
          $sqlCheck="SELECT * FROM  payment_partner_providers 
          WHERE `partner_providers_domain_url` = :target AND payment_date BETWEEN DATE_SUB(NOW(), INTERVAL 7 DAY) AND NOW()";
        $stmt_ads = $pdo->prepare($sqlCheck);

        $sqlCheck_r = str_replace(":target",$target,$sqlCheck);
        echo "<br>sqlCheck_r: " . $sqlCheck_r . "<br>";
        ...
        foreach ($ads as $ad) {
            ...
            $data = array(
                ...
                'id' => $ad['id'],
                'partner_providers_domain_url' => $ad['partner_providers_domain_url'],
                'email_provider' => $ad['email_provider'],
                'nominal' => $ad['nominal'],
